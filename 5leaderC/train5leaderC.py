...
def parse_args():
    ...
    parser.add_argument('--model_saved', type=str, help='model saved name (default: saved_model.h5)')
...
def shuffle_train_test(input_data_array, input_label_array, ratio=0.1):
    train_num = input_data_array.shape[0]

    data_shuffle, label_shuffle = shuffle_arrays(input_data_array,input_label_array)
...
def train(config, checkpoint_dir = None):
    ...
    trainData, trainLabel, test_data, test_label = shuffle_train_test(train_data_array, train_label_array, ratio=0.1)
    ...
    for fold, (train_index, valid_index) in enumerate(kfold.split(trainData)):
        ...
        # kfold split
        train_data, valid_data = trainData[train_index], trainData[valid_index]
        train_label, valid_label = trainLabel[train_index], trainLabel[valid_index]
        ...
        for e in range(1, config.epoch+1):
            ...
            if early_stopping.early_stop:
                ...
                test(config, test_dataloader, fold+1)
                break 
...
def test(config, test_dataloader, fold):
    ...
    df = pd.DataFrame({'Predict':predictions,'Label':labels,'Seqs':seqs})
    df.to_csv('predict_result.csv', index=False)
    df.to_csv(f'predict_result_{fold}.csv', index=False)
    train_utils.all_class_metrics(predictions, labels)
    train_utils.roc_curves(predictions, labels, fold)
    train_utils.pr_curves(predictions, labels, fold)
