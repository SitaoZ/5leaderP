...
if __name__ == '__main__':
    regress = pd.read_csv('../regression/regression_data.csv')
    positive = regress[['Locus','Seq']]
    positive.loc[:,'Label'] = 1
    positive = positive.drop_duplicates(subset=['Locus'])
    # negative
    negative = pd.read_csv('negative/negative_data.csv', header=None)
    negative.columns = ['Locus', 'Seq']
    negative.loc[:,'Label'] = 0
    # negative = negative.drop_duplicates(subset=['Locus'])
