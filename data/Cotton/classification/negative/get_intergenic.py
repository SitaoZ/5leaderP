...
fasta = {}
for record in SeqIO.parse('gab.fa', 'fasta'):
...
with open('All_isoforms_V1.0.gff', 'r') as f:
    for line in f:
        ...
        if len(line) == 0:
            continue
        array = line.split('\t')
        # Chr01   StringTie       gene    1       10332   1000    -       .       ID=MSTRG.4;
        if array[2] == 'gene':
        ...
        if i == 0 :
            ...
            if strand == "+":
                ...
                if intergenic > 2*threshold:
                    if (intergenic - 3*threshold) > 0:
                        r_start = gene[2] + 3*threshold
                        if r_start > len(fasta[chrom]) or r_start+512 > len(fasta[chrom]):
                            continue
                        for r in random.sample(range(r_start, r_start+512), 3):
                            seq = fasta[chrom][r-512 : r]
                            seq = reverse_comp(seq)
