...
class AttenNet(nn.Module):
    ...
    def __init__(self, num_classes=1, attention=True, normalize_attn=True, init_weights=False):
        ...
        # conv blocks
        self.conv1 = self._make_layer(4, 64, 1, kernel_size=7, padding=3)
    ...
    def _make_layer(self, in_features, out_features, blocks, kernel_size=3, padding=1, pool=False):
        ...
        for i in range(blocks):
            conv1d = nn.Conv1d(in_channels=in_features, out_channels=out_features, kernel_size=kernel_size, padding=padding, bias=False)
...
class RegResNet(nn.Module):
    def __init__(self, l1, l2):
        ...
        # 512/2/2/2 = 64 128
        self.layer1 = self.make_layer(channel_in=4, channel_out=128, block_num=2, kernel_size=7, padding_size=3,
                                      stride=1)
        # 64/2/2/2 = 8 32
...
class Attention(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, padding):
        ...
        self.query_conv = nn.Conv1d(in_channels, out_channels, kernel_size, padding=padding)
        self.key_conv = nn.Conv1d(in_channels, out_channels, kernel_size, padding=padding)
        self.value_conv = nn.Conv1d(in_channels, out_channels, kernel_size, padding=padding)
    ...
    def forward(self, x):
        ...
        out = torch.matmul(m, value)
        out = self.gamma * out + x
        return out
class RegResAttenNet(nn.Module):
    def __init__(self, l1, l2):
        super(RegResAttenNet, self).__init__()
        # 512/2/2/2 = 64 128
        self.layer1 = self.make_layer(channel_in=4, channel_out=128, block_num=3, kernel_size=7, padding_size=3,
                                      stride=1)
        # 64/2/2/2 = 8 32
        self.layer2 = self.make_layer(channel_in=128, channel_out=256, block_num=3, kernel_size=3, padding_size=1,
                                      stride=1)
        # 8/2/2/2 = 1 8
        # batch channel length
        # self.dropout = nn.Dropout(0.5)
        # self.leak = nn.LeakyReLU(0.01)
        self.Attention = Attention(256, 256, 3, 1) # batch, 512 , 8
        self.flatten = torch.nn.Flatten()
        self.fc1 = nn.Linear(256*8, l1, bias=True)
        self.fc2 = nn.Linear(l1, l2, bias=True)
        self.fc3 = nn.Linear(l2, 1, bias=True)
    def forward(self, x):
        x = self.layer1(x)
...
class RegCnnNet(nn.Module):
    ...
    def __init__(self, l1, l2):
        ...
        self.layer1 = Sequential(
            nn.Conv1d(4, 64,  # input channel, output channel
                      kernel_size=7,
                      stride=1,
                      dilation=1,
                      groups=1,
...
if __name__=='__main__':
    ...
    # model = RegLstmNet(256, 128)
    # model = AttenNet(num_classes=1)
    # model = RegAttenNet()
    # model = RegResNet(128, 64)
    # print(model.model_name())
    model = RegCnnNet(128, 64)
    summary(model)
