...
# model.load_weights("./models/classification.h5")

# test_set = np.load('./data/test_class_set.npy')
# version 20230110
# test_set = np.load('./final_version/test_class_data.npy')
# test_set = np.load('../../TSARC/test_class_data.npy')
# test_answers = np.load('./data/test_class_answers.npy')
# test_answers = np.load('./final_version/test_class_label.npy')
# test_answers = np.load('../../TSARC/test_class_label.npy')

# predictions = model.predict(test_set)
# metrics.all_class_metrics(predictions, test_answers)
# metrics.roc_curves(predictions, test_answers)
# metrics.pr_curves(predictions, test_answers)
...
model = models.regression_model((512, 4))
# model = models.regression_model(train_set[0].shape)
# model.fit(train_set, train_answers, batch_size=64, epochs=5, verbose=2, validation_split=0.1)
model.load_weights("./models/regression.h5")
# test_set = np.load('./data/test_regr_set.npy')
test_set = np.load('../5leaderL/test_data.npy')
# test_answers = np.load('./data/test_regr_answers.npy')
test_answers = np.load('../5leaderL/test_label.npy')

predictions = model.predict(test_set)

predictions = abs(predictions - 256)
...
kf = KFold(n_splits=5)
for fold, (train_index, valid_index) in enumerate(kf.split(predictions)):
    train_data, valid_data = predictions[train_index], predictions[valid_index]
    train_label, valid_label = test_answers[train_index], test_answers[valid_index]
    print(fold, 'MSE:', metrics.mse(valid_data, valid_label))
    print(fold, 'MAE/RMSE:', metrics.rmse(valid_data, valid_label))
